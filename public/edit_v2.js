...
document.querySelectorAll(".colorOption").forEach(el => {
    el.onclick = () => {
        ...
        activateColor(el); currentTool = "marker"; penMode = false;
        drawCanvas();
    };
});
...
document.querySelectorAll(".arrowColorOption").forEach(el => {
    el.onclick = () => {
        ...
        activateColor(el); currentTool = "arrow"; penMode = false;
        drawCanvas();
    };
});
...
document.querySelectorAll(".textColorOption").forEach(el => {
    el.onclick = () => {
        ...
        activateColor(el); currentTool = "text"; penMode = false;
        drawCanvas();
    };
});
...
document.querySelectorAll(".penColorOption").forEach(el => {
    el.onclick = () => {
        ...
        activateColor(el); currentTool = "pen"; penMode = true;
        drawCanvas();
    };
});
...
/* ============================================================
   ヒットテスト
============================================================ */
let draggingMarker = null, draggingArrow = null;
    ...
    if (selectedObject && selectedType) {
        ...
        if (hitScaleHandle(x, y, b)) {
            ...
            transformStart = {
                ...
                originalMarkerSize: selectedType === "marker" ? selectedObject.size : null
            };
            saveHistory();
            return;
        }
        ...
        if (hitRotateHandle(x, y, b)) {
            ...
            transformStart = {
                x, y,
                centerX: selectedObject.x,
                centerY: selectedObject.y,
        ...
        if (selectedType === "marker") {
            if (x >= b.x && x <= b.x + b.w && y >= b.y && y <= b.y + b.h) {
                clearTool();
                saveHistory();
                draggingMarker = selectedObject;
                dragOffsetX = x - draggingMarker.x;
                dragOffsetY = y - draggingMarker.y;
                return;
            }
        }
    }
...
canvas.addEventListener("mousemove", e => {
    ...
    if (transformMode === "scale" && selectedObject && transformStart) {
        ...
        if (selectedType === "marker") {
            selectedObject.size = Math.max(5, (transformStart.originalMarkerSize || selectedObject.size) + d / 4);
        }
